    ...
    public function statusBadge(): Attribute
    {
        return new Attribute(function(){
            ...
            if($this->status == Status::TICKET_OPEN){
                $html = '<span class="badge badge-two badge--success">'.trans("Open").'</span>';
            }
            elseif($this->status == Status::TICKET_ANSWER){
                $html = '<span class="badge badge-two badge--primary">'.trans("Answered").'</span>';
            }

            elseif($this->status == Status::TICKET_REPLY){
                $html = '<span class="badge badge-two badge--warning">'.trans("Customer Reply").'</span>';
            }
            elseif($this->status == Status::TICKET_CLOSE){
                $html = '<span class="badge badge-two badge--dark">'.trans("Closed").'</span>';
            }
            return $html;
        });
    }
