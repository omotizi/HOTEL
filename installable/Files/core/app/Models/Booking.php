...
class Booking extends Model
{
        ...
        return new Attribute(
            function () {
                if (now() >= $this->check_in && $this->status == Status::BOOKING_ACTIVE) {
                    $class = "badge-two badge--success";
                    $text = 'Running';
                } elseif (now() < $this->check_in && $this->status == Status::BOOKING_ACTIVE) {
                    $class = "badge-two badge--warning";
                    $text = 'Upcoming';
                } elseif ($this->status == Status::BOOKING_CANCELED) {
                    $class = "badge-two badge--danger";
                    $text = 'Canceled';
                } else {
                    $class = "badge-two badge--dark";
                    $text = 'Checked Out';
                }
    ...
    {
        $totalFare = $this->bookedRooms()->sum('fare');
        return $this->tax_charge * 100 / ($totalFare > 0 ? $totalFare : 1);
    }
