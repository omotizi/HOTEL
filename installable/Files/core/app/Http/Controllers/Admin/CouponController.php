...
class CouponController extends Controller
{
    public function index()
    {
        ...
        $coupons   = Coupon::withCount('appliedCoupons')->searchable(['coupon_name', 'coupon_code'])->paginate(getPaginate());
    ...
    {
        $coupon = Coupon::whereId($id)->with(['roomTypes'])->firstOrFail();
    ...
    public function save(Request $request, $id)
    {
        ...
        if ($id) {
            $coupon->roomTypes()->sync($request->room_types);
        } else {
            $coupon->roomTypes()->attach($request->room_types);
        }
    ...
    public function roomTypeForCoupon(Request $request)
    {
    ...
    public function appliedCoupon($id)
    {
        $coupon = Coupon::findOrFail($id);
        $appliedCoupons = AppliedCoupon::searchable(['booking:booking_number','user:username'])->with(['booking:id,booking_number', 'coupon:id,coupon_code','user:id,username'])->where('coupon_id', $id)->paginate(getPaginate());
        $pageTitle = 'Applied Coupon - '.$coupon->coupon_code;
        return view('admin.coupons.apply', compact('pageTitle','appliedCoupons'));
    }
