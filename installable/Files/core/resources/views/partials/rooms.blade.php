...
<div class="col-lg-12 parentDiv parentContainer-{{ $roomType->id }}">
                                    ...
                                    <div class="d-flex w-100 flex-wrap gap-2">
                                        @foreach ($rooms as $room)
                                            ...
                                            @if ($booked)
                                                ...
                                                @endphp
                                            @else
                                                <button class="btn btn--primary btn-sm room-btn available" data-booked_status="0" data-unit_fare="{{ getAmount($room->roomType->roomFare()) }}"  data-room_type_id="{{ $room->roomType->id }}" data-date="{{ $checkIn->format('m/d/Y') }}" room="room-{{ $room->room_number }}" data-room="{{ $room }}">
                                                    {{ $room->room_number }}
                                                </button>
                                            @endif
                                        @endforeach
    ...
    <script>
        (function($) {
            ...
            var bookingRequestId = @json($roomType->id);
