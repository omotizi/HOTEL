...
@foreach ($bookingRequest->roomBookingRequest ?? [] as $item)
    @php
        ...
        $roomType = App\Models\RoomType::find($item->room_type_id);
        $rooms = App\Models\Room::active()
            ...
            ->with([
                ...
                'roomType:id,name,fare',
            ])
            ->get();
    @endphp
    <div class="col-lg-12 parentDiv parentContainer-{{ $item->id }}">
                        ...
                        <tbody class="room-table">
                            ...
                            <tr>
                                <td class="text-center" colspan="2">
                                    ...
                                    <small class="text--info">@lang('Total Room'): {{ $numberOfRooms }}</small>
                                </td>
                            </tr>
                            ...
                            @while ($checkIn < $checkout)
                                <tr>
                                    ...
                                    <td class="room-column @if ($item->check_in == $checkIn->format('Y-m-d')) first-column-rooms @endif" data-number_of_rooms="{{ $numberOfRooms }}">
                                        <div class="d-flex w-100 flex-wrap gap-2">
                                            @foreach ($rooms as $room)
                                                ...
                                                @if ($booked)
                                                    ...
                                                    @endphp
                                                @else
                                                    <button class="btn btn--primary btn-sm room-btn available" data-booked_status="0" data-date="{{ $checkIn->format('m/d/Y') }}" data-room="{{ $room }}" data-room_type_id="{{ $room->roomType->id }}" data-unit_fare="{{ getAmount(@$item->unit_fare) }}" room="room-{{ $room->room_number }}">
                                                        {{ $room->room_number }}
                                                    </button>
                                                @endif
                                            @endforeach
        ...
        <script>
            (function($) {
                ...
                var bookingRequestId = @json($item->id);

                selectRooms(selectedRooms, bookingRequestId);
            })(jQuery);
        </script>
    </div>
@endforeach
