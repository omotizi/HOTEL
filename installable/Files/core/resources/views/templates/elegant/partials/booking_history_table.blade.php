    ...
    <tbody>
        @forelse ($bookings as $booking)
            <tr>
                ...
                <td>
                    <span class="@if ($due) text--danger @else text--success @endif">
                        {{ showAmount($due) }}
                    </span>
                </td>
                ...
                <td>
                    <div class="d-flex justify-content-end flex-wrap align-items-center gap-2">
                        <a class="btn btn--sm btn-outline--info @if ($due == 0) disabled @endif" href="{{ route('user.booking.payment', $booking->id) }}">
                            <i class="las la-wallet"></i> @lang('Pay Now')
                        </a>

                        <a class="btn btn--sm btn-outline--base" href="{{ route('user.booking.details', $booking->booking_number) }}">
