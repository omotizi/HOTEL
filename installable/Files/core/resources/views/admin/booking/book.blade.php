...
@section('panel')
    <div class="row">
        @can('owner.room.search')
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('admin.room.search') }}" class="formRoomSearch" method="get" data-tax-charge="{{ gs('tax') }}">
    ...
    <div class="row gy-4 mt-4 booking-wrapper d-none">
        <div class="col-lg-8">
        ...
        <div class="col-lg-4">
            <div class="card sticky-card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.room.book') }}" class="booking-form" id="bookingForm" method="POST">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="form-group">
                                    ...
                                    <input type="number" min="1" name="total_adult" class="form-control" required>
                                </div>
                            </div>
                            ...
                            <div class="orderList d-none">
                                ...
                                <div class="d-flex justify-content-between align-items-center border-top p-2 discountLi d-none">
                                    <span>@lang('Tax')</span>
                                    <span class="showTotalTax"></span>
                                </div>

                                <div class="d-flex justify-content-between align-items-center border-top p-2">
                                    <span>@lang('Tax')</span>
                                    <span class="showTaxAmount"></span>
                                </div>

                                <div class="d-flex justify-content-between align-items-center border-top p-2">
                                    <span class="showCouponTitle">@lang('Coupon')</span>
                                    <div class="couponInput">
                                        <div class="input-group">
                                            <input type="text" name="coupon_code" class="form-control coupon-input-width" placeholder="Coupon Code">
                                            <button type="button" class="input-group-text bg--primary addCoupon"><i class="las la-check-circle"></i></button>
                                        </div>
                                    </div>
                                    <div class="d-flex align-items-center gap-3 showCouponBox d-none">
                                        <span class="showCouponCode"></span>
                                        <button type="button" class="btn btn-sm btn-danger removeCouponBtn"><i class="las la-times"></i></button>
                                    </div>
                                </div>

                                <div class="d-flex justify-content-between align-items-center border-top p-2 discountAmountBox d-none">
                                    <span>@lang('Discount Amount')</span>
                                    <span class="showDiscountAmount"></span>
                                </div>
                                <div class="d-flex justify-content-between align-items-center border-top p-2">
...
@push('style')
    <style>
        ...
        .coupon-input-width.form-control {
...
@push('script')
    <script>
        ...
        function getAvailableRoom() {
            let searchDate = $('input[name=date]').val();
            let roomType = $('select[name=room_type]').val();
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('admin.room.available') }}",
                data: {
        ...
        $(document).on('click', '.removeRoomTypeBtn', function() {
            ...
            $.ajax({
                type: "post",
                url: "{{ route('admin.room.session.data.update') }}",
                data: {
                    _token: "{{ csrf_token() }}",
                    room_type: roomType
                },
    ...
    <script>
        (function($) {
            ...
            $('.addCoupon').on('click', function() {
                ...
                if (!couponCode) {
                    notify('error', 'Enter coupon code');
                    return false;
                }

                let gestType = $('[name=guest_type]').val();
                let userEmail = null;
                ...
                $.each(subtotal, function(index, element) {
                    totalAmount += parseFloat($(element).attr('sub_total'));
                ...
                $.each(selectedRooms, function(i, selectedRoom) {
                    roomTypeIds.push($(selectedRoom).data('room_type_id'));
                });
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.room.apply.coupon') }}",
                    data: {
                    ...
                    success: function(response) {
                        if (response.status == 'success') {
                            $('.showCouponBox').removeClass('d-none');
                            $('.discountAmountBox').removeClass('d-none');
                            $('.couponInput').addClass('d-none');
                            $('.showCouponCode').text(response.coupon_code);
                            $('.showTaxAmount').text(response.total_tax);
                            $('.showDiscountAmount').text(response.discount_amount);
                            $('.grandTotalFare').text(response.total_amount_with_tax);
                            $('.showCouponTitle').text('Used Coupon');
            ...
            $('.removeCouponBtn').on('click', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.room.remove.coupon') }}",
                    data: {
                        'amount': amount,
                        '_token': '{{ csrf_token() }}',
                    },
                    success: function(response) {
                        if (response.status == 'success') {
                            ...
                            $('.discountAmountBox').addClass('d-none');
                            $('.couponInput').removeClass('d-none');
                            $('.showCouponCode').text('');
                            $('.showTaxAmount').text(response.total_tax);
                            $('.showDiscountAmount').text('0.00');
                            $('.grandTotalFare').text(response.total_amount_with_tax);
                            $('[name=total_amount]').val(response.total_amount_with_tax_value)
                            $('[name=coupon_code]').val('')
                            $('.showCouponTitle').text('Coupon');
                            notify('success', response.message);
                        } else {
                            if (Array.isArray(response.error)) {
                                response.error.forEach(element => {
