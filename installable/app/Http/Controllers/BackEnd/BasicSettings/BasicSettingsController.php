...
class BasicSettingsController extends Controller
{
  ...
  public function updateThemeVersion(Request $request)
  {
    ...
    if($request->theme_version == 'theme_three' || $request->theme_version == 'theme_four' || $request->theme_version == 'theme_five' ){
  ...
  public function updateFavicon(Request $request)
  {
    ...
    $data = DB::table('basic_settings')->select('favicon')
      ->where('uniqid', 12345)
      ->first();
    ...
    copy(public_path($faviconURL), $directory . $iconName);
  ...
  {
    $data = DB::table('basic_settings')->select('logo')
      ->where('uniqid', 12345)
      ->first();
  ...
  public function updateLogo(Request $request)
  {
    ...
    copy(public_path($logoURL), $directory . $logoName);
  ...
  public function updateBreadcrumb(Request $request)
  {
    ...
    $data = DB::table('basic_settings')->select('breadcrumb')
      ->where('uniqid', 12345)
      ->first();

    // second, delete the previous breadcrumb from local storage
    if (!is_null($data->breadcrumb) && file_exists(public_path('assets/img/') . $data->breadcrumb)) {
    ...
    copy(public_path($breadcrumbURL), $directory . $breadcrumbName);
  ...
  public function updateMaintenance(Request $request)
  {
    ...
    if ($request->filled('maintenance_img')) {
      ...
      if (
        ...
        file_exists(public_path('assets/img/') . $data->maintenance_img)
      ) {
      ...
      copy(public_path($maintenanceImgURL), $directory . $maintenanceImgName);
    }
  ...
  {
    $data = DB::table('basic_settings')->select('footer_logo')
      ->where('uniqid', 12345)
      ->first();
  ...
  public function updateFooterLogo(Request $request)
  {
    ...
    if (!is_null($data->footer_logo) && file_exists(public_path('assets/img/') . $data->footer_logo)) {
    ...
    copy(public_path($footerLogoURL), $directory . $footerLogoName);
  ...
  public function updatepreloader(Request $request)
  {
    ...
    $data = DB::table('basic_settings')->select('preloader')
      ->where('uniqid', 12345)
      ->first();

    // second, delete the previous preloader from local storage
    if (!empty($preloader) && !is_null($data->preloader) && file_exists(public_path('assets/img/') . $data->preloader)) {
      unlink(public_path('assets/img/') . $data->preloader);
    }

    if ($request->filled('preloader')) {
      $filename = uniqid() . '.' . $extPreloader;

      @copy(public_path($preloader), public_path('assets/img/') . $filename);
    }
